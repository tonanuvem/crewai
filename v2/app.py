...
@st.cache_resource
def get_llm(model_choice: str, custom_model: str, temperature: float):
    """
    Cria instância do LLM nativo do CrewAI.
    model_choice: rótulo selecionado no UI
    custom_model: string exata do modelo caso o usuário queira sobrescrever
    """
    ...
    # Mapeamento de rótulos para nomes aceitos pela API (ajuste conforme sua conta)
    MODEL_MAP = {
        "Gemini 1.5 Flash (rápido/gratuito)": "gemini-1.5-flash-latest",
        "Gemini 1.5 Pro (mais potente)": "gemini-1.5-pro-latest",
        "Gemini 2.0 Flash (experimental)": "gemini-2.0-flash-exp",
        # Adicione outras mapeamentos se desejar
    }
    ...
    if custom_model and custom_model.strip():
        ...
        model_name_to_use = MODEL_MAP.get(model_choice, "gemini-1.5-flash-latest")
...
def main():
    ...
    with st.sidebar:
        ...
        model_option = st.selectbox(
            "🧠 Modelo Gemini (mapeado)",
            [
                "Gemini 1.5 Flash (rápido/gratuito)",
                "Gemini 1.5 Pro (mais potente)",
                "Gemini 2.0 Flash (experimental)",
            ],
            help="Escolha um dos modelos mapeados. Use 'Modelo custom' para informar o nome exato usado pela API."
        )
        
        custom_model = st.text_input(
            "Modelo custom (opcional)",
            placeholder="Ex: gemini-1.5-flash-latest ou gemini-2.0-alpha-xyz (use se precisar do nome exato)",
            help="Se preenchido, este valor será usado diretamente em vez do mapeamento acima."
        )
        ...
        # Informação sobre modelos
        st.info("💡 Se tiver erro 404, tente usar o nome exato do modelo (campo 'Modelo custom').")
            ...
            if start_button:
                ...
                with progress_container:
                    ...
                    try:
                        ...
                        llm = get_llm(model_option, custom_model, temperature)
                        ...
                        # Atualiza os placeholders de progresso após a conclusão
                        for agent in agents:
                        ...
                        st.session_state.results = {
                            ...
                            'model': custom_model.strip() if custom_model.strip() else model_option,
                            'temperature': temperature
                        }
